...
In "setuptools" I include installation methods using setuptools or its
variants, meaning pip_ and setuptools_ itself.
...
* Installation by ``pip`` from binary ``.whl`` file (any platform).
...
``easy_install my_package_version.egg``.  Pip and wheels work in a very
similar way.
